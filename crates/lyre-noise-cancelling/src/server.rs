...
use crate::{RnnoiseFrameProcessor, RNNOISE_CHANNELS, RNNOISE_FRAME_SIZE, RNNOISE_SAMPLE_RATE_HZ};
...
impl RnnoiseNoiseCanceller {
    pub fn new(config: NoiseCancellationConfig) -> Self {
        Self {
            config,
            processor: RnnoiseFrameProcessor::new(),
        }
    }
...
impl NoiseCanceller for RnnoiseNoiseCanceller {
    fn process_frame(
        ...
        let output = self.processor.process_samples(frame.samples);

        Ok(NoiseFrameOutput {
            samples: output.samples,
            voice_activity_probability: Some(output.voice_activity_probability),
        })
    }
}
...
pub(crate) struct NoiseConfigKey {
