    ...
    I: Interceptor + 'static,
{
    ...
    async fn add_ice_candidate(&self, candidate: RTCIceCandidateInit) -> Result<()> {
        ...
        // Trickle ICE updates the sans-I/O core after negotiation has already
        // armed the driver's timeout. Wake the driver so it polls the new
        // candidate immediately instead of waiting until the check timer fails.
        self.inner
            .driver_event_tx
            .try_send(PeerConnectionDriverEvent::WriteNotify)
            .map_err(|e| Error::Other(format!("{:?}", e)))
