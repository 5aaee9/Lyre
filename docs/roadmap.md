...
- Browser microphone capture now exposes persisted echo cancellation, auto gain control, and browser noise suppression settings; browser noise suppression defaults on, and disabled browser DSP constraints are requested with exact false.
...
- Frontend audio diagnostics now include local/server ICE candidate counts, the last server candidate batch size, candidate timestamps, and candidate add errors so sender-side `ICE new` stalls can be isolated without browser devtools.
...
- Frontend default server noise cancelling now starts with DPDFNet using the `dpdfnet2_48khz_hr` model for new settings snapshots.
...
- Frontend root route now exposes the shared Settings dialog before joining so users can adjust identity, relay denoise, device, and advanced audio preferences from the entry screen.
...
- Frontend audio diagnostics now style manual refresh as a standard button, scroll their own long content, and no longer stretch the voice channel card when enabled.
